...
function customerPayload(row, source) {
  ...
  return {
    ...
    whatsapp: onlyDigits(row.whatsapp || row.whats_app || row.celular || row.telefone_whatsapp) || null,
    email: clean(row.email || row.e_mail)?.toLowerCase() || null,
    endereco: clean(row.endereco),
    endereco_completo: clean(row.endereco),
    bairro: null,
    cidade: clean(row.cidade),
    uf: clean(row.uf || row.estado)?.toUpperCase().slice(0, 2) || null,
    categoria,
