...
-- Seguranca da Data API: o LetecLog acessa tabelas somente pelo backend.
-- Mantenha este bloco no fim do setup para que tabelas novas nao nascam publicas.
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format('REVOKE ALL PRIVILEGES ON TABLE %I.%I FROM anon, authenticated', target.schemaname, target.tablename);
    EXECUTE format('GRANT ALL PRIVILEGES ON TABLE %I.%I TO service_role', target.schemaname, target.tablename);
  END LOOP;
END $$;
